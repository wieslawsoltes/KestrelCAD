...
(function (root) {
    ...
    class App {
        ...
        insertData(data) { K.validateProject(data); const factor = UNIT_MM[data.units] / UNIT_MM[this.doc.units], mapping = new Map(), ids = []; this.doc.transaction('Insert drawing', () => { for (const l of data.layers) {
            let target = this.doc.layers.find(x => x.name.toLowerCase() === l.name.toLowerCase());
            if (!target) {
                target = { ...l, id: K.uid('layer') };
                this.doc.layers.push(target);
            }
            mapping.set(l.id, target.id);
        } for (const original of data.entities) {
            const e = G.transform(original, M.scale(factor));
            delete e.id;
            delete e.group;
            delete e.groupName;
            e.layer = mapping.get(original.layer);
            ids.push(this.doc.add(e).id);
        } this.doc.selection = new Set(ids); }); this.log('Insert', ids.length + ' entities inserted at their original coordinates; unit conversion ×' + factor + '. Use Move to place them.'); }
