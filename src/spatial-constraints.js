...
function validate(data){
 ...
 for(const c of s.constraints){
  ...
  if(['fastened','slider'].includes(c.type))for(const r of refs(c)){if(V.len(V.cross(V.norm(r.axis||[0,0,1]),V.norm(r.xaxis||[1,0,0])))<1e-7)throw Error('A mating frame needs two independent axes.');}
...
function compile(doc){
 ...
 const origin=V.lerp(low,high,.5),scale=Math.max(1,V.dist(low,high),...active.filter(c=>['distance','point-plane','plane-mate'].includes(c.type)).map(c=>Math.abs(targets.get(c.id))));
 ...
 function residualFor(c,ctx){
  ...
  switch(c.type){
   ...
   case 'hinge':return [...d,...V.sub(z(),w())];
   case 'slider':return [...orient(),...V.cross(d,w())];
   case 'fastened':return [...orient(),...d];
   default:throw Error('Unknown spatial constraint.');
  }
 }
 const residual=x=>{const ctx=context(x),r=active.flatMap(c=>residualFor(c,ctx));if(r.length>limits.scalars||r.some(v=>!Number.isFinite(v)))throw Error('Invalid or oversized spatial residual system.');return r;};
 ...
 return {x,residual,valid,apply,active,context,residualFor,scale};
}
...
function solve(doc,{apply=true,maxIterations=limits.iterations,timeLimit=1500}={}){
 ...
 for(;iterations<maxIterations&&max(r)>1e-9&&x.length&&Date.now()-start<timeLimit;iterations++){
  const j=jacobian(sys.residual,x,r),n=x.length,a=Array.from({length:n},()=>Array(n).fill(0)),b=Array(n).fill(0);
 ...
 const converged=max(r)<=1e-9&&sys.valid(x),rk=rank(jacobian(sys.residual,x,r),x.length),ctx=sys.context(x);
 const report={converged,status:converged?(rk===x.length?'fully-constrained':'under-constrained'):'failed-to-converge',variables:x.length,equations:r.length,rank:rk,degreesOfFreedom:x.length-rk,redundantEquations:r.length-rk,maxResidual:max(r),lengthScale:sys.scale,iterations,
...
K.SpatialConstraints={TYPES,DIM,RIGID,FLEX,limits,state,validate,solve,enforce,add,anchor,frame,removeReferences,convertUnits,copyInto};
})(globalThis);
